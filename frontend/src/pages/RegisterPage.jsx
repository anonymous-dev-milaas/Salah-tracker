...
export default function RegisterPage() {
  const [form, setForm] = useState({ email: '', username: '', password: '', city: 'Malappuram' })
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        ...
        <form onSubmit={handle} className="glass rounded-2xl p-6 space-y-4">
          <h2 className="font-semibold text-slate-200 text-lg">Register</h2>
          {error && <p className="text-rose-400 text-sm bg-rose-400/10 rounded-xl px-3 py-2">{error}</p>}
          {[
            { label: 'Display Name', key: 'username', type: 'text', placeholder: 'Your name' },
            { label: 'Email', key: 'email', type: 'email', placeholder: 'you@example.com' },
            { label: 'Password', key: 'password', type: 'password', placeholder: '8+ characters' },
            { label: 'City', key: 'city', type: 'text', placeholder: 'Malappuram' },
